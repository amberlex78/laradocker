<x-layouts.auth :title="'Verify email'">
    ...
    <div class="mt-8 flex flex-col gap-4">
        ...
        <form class="text-center" method="POST" action="{{ route('logout') }}">
            @csrf
            <button class="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 transition hover:text-slate-950 dark:text-slate-400 dark:hover:text-white" type="submit"><x-icon name="log-out" class="h-4 w-4" />Log out</button>
        </form>
    </div>
</x-layouts.auth>
