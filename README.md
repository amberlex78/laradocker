...
Щоб розгорнути ще один незалежний clone цього Laravel-проєкту, склонуй його в окрему директорію та створи власний `.env`. Але перед тим як запустити його, треба зупинити поточний проєкт. Якщо поточний проєкт має залишатися запущеним, у другому `.env` потрібно вказати інші host-порти.:
